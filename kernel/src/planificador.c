...
// ORGANIZA LOS PCB SEGÚN ME LLEGAN, PUEDE ENVIARLO AL SHORT O LONG o manejarlos internamente por si llegan bloqueados.
...
op_code_reception* long_term_scheduler(config_current_process* current_config_process) {
	...
	t_queue* queue_local_pcb_new = queue_create();	// TODO: LA COLA YA DEBERÍA EXISTIR, NO CREARSE ACA
	...
	// TODO: Debería ser un while que verifique todo el tiempo si puede ejecutar un nuevo pcb
	// Reviso si la multiprogramación permite planificar mas de uno
	int sem_value;
	sem_getvalue(&FLAG_MULTIPROGRAMACION, &sem_value);
	while (sem_value < 0) {
		// consumo 1 del flag de multiprogramación
	...
	if (current_config_process->current_process->pcb->state_pcb == EXIT) {
		log_info(logger, "El proceso %d terminó su ejecución", current_config_process->current_process->pcb->pid);
		// libero 1 del flag de multiprogramación
		sem_post(&FLAG_MULTIPROGRAMACION);
		// BORRAR ID DE EJECUCIÓN
		// liberar recursos de pcb
		//  devolver motivo de finalización
	}

	return OK;
}

// Este planificador de corto plazo recibe un pcb, algoritmo de planificación, y la cola global de pcb |READY|, logger, int server_cpu
void short_term_scheduler(global_config_kernel* gck, process* process) {
	// DETECTAMOS EL ESTADO ACTUAL DE EJECUCIÓN, SI NO SE ESTA EJECUTANDO NADA HAGO LO DE ABAJO
	/*
